...
#define MMC0P17_MOUNT_POINT "/android_esp"
#define MMC0P17_BDEV_NAME  "mmc0p15"
...
static int mmc_ls_partition(void) {
    status_t status;
    struct dirent ent;
    dirhandle *handle;

    status = fs_mount(MMC0P17_MOUNT_POINT, "fat", MMC0P17_BDEV_NAME);
    if (status != NO_ERROR) {
        printf("Mount failed: %d\n", status);
        return status;
    }

    status = fs_open_dir(MMC0P17_MOUNT_POINT "/", &handle);
    if (status != NO_ERROR) {
        printf("Open failed: %d\n", status);
        fs_unmount(MMC0P17_MOUNT_POINT);
        return status;
    }

    for (;;) {
        status = fs_read_dir(handle, &ent);
    ...
    status = fs_close_dir(handle);
    if (status != NO_ERROR) {
        LOGF("failed to close the target directory: %d\n", status);
        return status;
    }

    status = fs_unmount(MMC0P17_MOUNT_POINT);
    if (status != NO_ERROR) {
        LOGF("failed to unmount mmc on mount point (%s): %d\n",
             MMC0P17_MOUNT_POINT, status);
        return status;
    }
...
struct mmc_test tests[] = {
    {"read", mmc_read_test},
    {"ls_partition", mmc_ls_partition},
    { NULL, NULL },
};

int mmc_tests(int argc, const console_cmd_args *argv) {
    for (struct mmc_test *t = tests; t->name; t++) {
        status_t status = t->test_fn();
        if (status != NO_ERROR)
